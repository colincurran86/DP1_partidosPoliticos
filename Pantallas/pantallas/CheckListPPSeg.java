...
public class CheckListPPSeg extends JDialog {
	...
	public static void main(String[] args) {
		try {
			CheckListPP dialog = new CheckListPP(ConfiguracionSeg.listaStrPP);
	...
	public CheckListPPSeg(  String[] strs ) {
	    ...
	    printButton.addActionListener(new ActionListener() {
	      public void actionPerformed(ActionEvent e) {
	        ...
	        for (int i = 0; i < n; i++) {
	          ...
	          if (item.isSelected()) {
	        	  entro=false;
	        	  for(int j=0;j<Carga.ppEscogidosSeg.size();j++){
	        		  if(Carga.ppEscogidosSeg.get(j).getNombre().compareTo(ConfiguracionSeg.listaPP.get(i).getNombre())==0){
	        			  entro=true;
	        			  break;
	        		  }
	        	  }
	        	  if(!entro) Carga.ppEscogidosSeg.add( ConfiguracionSeg.listaPP.get(i));
	  ...
	  private CheckableItem[] createData(String[] strs) {
	    ...
	    for (int i = 0; i < n; i++) {
	      ...
	      List<PartidoPolitico> pp=Carga.ppEscogidosSeg;
