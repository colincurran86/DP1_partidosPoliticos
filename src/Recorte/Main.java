...
public class Main {
	...
	public void main(String rutaPadrones, PartidoPolitico pp, String rutaFirma, String rutaHuella) {
		...
		// String formatearRutaBD =
		// "D:\\Users\\jemarroquin\\git\\DP1_partidosPoliticos\\src\\registro.nacional.v.1.xlsx"
		// ;

		String formatearRutaBD = u.formatearRuta(primeraFase.rutaExcel);
		llenarBDReniec(formatearRutaBD);

		String workingDir = System.getProperty("user.dir"); // nos evitamos el
															// problema de las
															// rutas :'v
		...
		for (final File fileEntry : folder.listFiles()) { // abre cada planillon
			...
			alturaX = rf.getX();
			yHuellas = rf.getYHuellas();
			rf.coordenadasFirma(padronJ);
			yFirmas = rf.getYFirmas();
			rf.coordenadaDNIyNombre(padronJ);
			yDNI = rf.getYDNI();
			yApellido = rf.getYApellido();
			yNombre = rf.getYNombre();

			// cropeamos los digitos del DNI
			List<String> dniLista = new ArrayList<String>();
			int numero, valorOriginal = yDNI, valorOriginalFirma = yFirmas, valorOriginalApellido = yApellido,
					valorOriginalNombre = yNombre;
			...
			for (int n = 0; n < 8; n++) { // iterando en las filas
				...
				for (int h = 0; h < 8; h++) {
					...
					// new
					// FileSaver(Copia1).saveAsPng("C:\\Users\\lenovo\\Desktop\\PUCP\\IMAGENEXXX\\"
					// + contador + ".jpg");
					contador++;
					// Prefs.blackBackground = false;

					if (h != 7)
						yDNI = rf.obtenerSiguienteEspacioDNI(yDNI + 2, alturaX + 5);
				...
				if (indice != -1) {
					Procesando.escribirTextArea(
							"lo encontre al dni !!" + dni + " - " + ReniecBD.lista.get(indice).getDni());
				...
				for (int i = 0; i < listaPersonasReniec.size(); i++)
					Procesando.escribirTextArea(
							"La lista de candidatos es la siguente: " + listaPersonasReniec.get(i).getDni());

				if (listaPersonasReniec.size() != 0) {
					try {
						// firmas
						listaPorcentajeFirma = AlgoritmoFirmas.procesarNuevo(listaPersonasReniec, n,
								rutaPlanillonEjecutandose, rutaFirma);
						int indiceCandidatoFirmas = candidatoFirmas(listaPorcentajeFirma);

						for (int i = 0; i < listaPersonasReniec.size(); i++)
							Procesando.escribirTextArea("Para el candiato: " + listaPersonasReniec.get(i).getDni()
									+ " Porcentaje de firmas es: " + listaPorcentajeFirma.get(i));
						Procesando.escribirTextArea("De todos los candidatos el mejor según firmas es: "
								+ listaPersonasReniec.get(indiceCandidatoFirmas).getDni());

						// if( listaPorcentajeFirma.get( indiceCandidatoFirmas )
						// > listaPorcentajeFirma.get(
						// listaPorcentajeFirma.size()-1 ) ) {
						if (elElegidoFinal == null) {
							elElegidoFinal = listaPersonasReniec.get(indiceCandidatoFirmas);
						}

						// huellas
						distanceBetweenSquares = 86;
						widthSquare = 150;
						heightSquare = 75;
						ImagePlus Copia2 = IJ.openImage(ruta3);
						Copia2.setRoi(yHuellas + 2, alturaFirma2 + 2, 150, 77);
						if (n != 7)
							alturaFirma2 = rf.obtenerSiguienteEspacioFirmas(yFirmas + 5, alturaFirma2 + 2);
						IJ.run(Copia2, "Crop", "");
						...
						for (int i = 0; i < listaPersonasReniec.size(); i++)
							Procesando.escribirTextArea("Para el candiato: " + listaPersonasReniec.get(i).getDni()
									+ " Porcentaje de huellas es: " + listaPorcentajeHuella.get(i));
						Procesando.escribirTextArea("De todos los candidatos el mejor según Huellas es: "
								+ listaPersonasReniec.get(indiceCandidatoHuellas).getDni());
	...
	public void llenarBDReniec(String rutaBD) {
		try {
			...
			while (rowIterator.hasNext()) {
				...
				if (nombre == null || nombre.getCellType() == Cell.CELL_TYPE_BLANK)
					break;
