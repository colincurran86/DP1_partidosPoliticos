 ...
 * @author sony
 */
class FastRetinaKeypointPattern {
    ...
    /**
     * Initializes a new instance of the FastRetinaKeypointPattern class.
    ...
    public FastRetinaKeypointPattern(int octaves, float scales) {
                ...
                for (int orientationIdx = 0; orientationIdx < Orientations; orientationIdx++)
                {
                    ...
                    for (int i = 0; i < 8; i++)
                    {
                        for (int k = 0; k < n[i]; k++)
                        {
                            ...
                            int sizeMax = (int)Math.ceil((radius[i] + sigma[i]) * scalingFactor * scale) + 1;
            ...
            for (int i = 0; i < 45; i++) {
                orientationPairs[i] = new OrientationPair();
            }
            ...
            for (int i = 0; i < 512; i++) {
                descriptionPairs[i] = new DescriptionPair(0, 0);
            }

            // default selected pairs
            for (int i = 0; i < descriptionPairs.length; i++)
                descriptionPairs[i] = allPairs.get(CV_FREAK_DEF_PAIRS[i]);
